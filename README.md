...
Before contributing, please make sure to familiarize yourself with our [code of conduct](https://github.com/db-ux-design-system/.github/blob/main/CODE-OF-CONDUCT.md).
...
- landing page layout
- `@orama/plugin-astro` is not yet compatible with Astro v6 (the `astro:build:done` hook no longer exposes `routes`). The `astro` peer dependency is pinned to `^5.0.0` until [oramasearch/orama#1020](https://github.com/oramasearch/orama/pull/1020) is merged and released, then upgrade `@orama/plugin-astro` and bump the peer dep to `^6.0.0`.
